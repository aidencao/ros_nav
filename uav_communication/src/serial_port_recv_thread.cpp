...
void SerialPortRecv::PubLandingInfo(const MsgHead *pMsg)
{
    RosTopicSendRecv::pub_landing_info();
}

//获取一个完整帧消息后分析
void SerialPortRecv::MessageAnalysis(const MsgHead *pMsg)
{
    if (pMsg->direction != FLIGHT_TO_GROUND_STATION)
    { //判断是否为飞机发来的 需要区分
    ...
    case MESSAGE_ANALYSIS_RECEIVE_LIDAR_NAV_ODOM:
        ...
        break;
    case MESSAGE_ANALYSIS_RECV_LANDING_INFO:
        PubLandingInfo(pMsg);
        break;
