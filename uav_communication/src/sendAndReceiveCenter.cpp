...
{
    static char init_point_msg[40];
    size_t idx = 0;

    //初始化GPS报文内容    
    init_point_msg[idx++] = 0xfe;                                              //header
    init_point_msg[idx++] = 0xff;
    *(UINT32*)(&init_point_msg[idx]) = (UINT32)(2 + 28 + CRC16_BYTES);         //len
    idx += sizeof(UINT32);
    init_point_msg[idx++] = GROUND_STATION_TO_FLIGHT;                          //direction
    init_point_msg[idx++] = MESSAGE_ANALYSIS_SET_INIT_POSE_POINT;                     //type
    double pos_x = initPoint[0];
    double pos_y = initPoint[1];
    double pos_z = initPoint[2];
    double ori_w = initPoint[3];
    double ori_x = initPoint[4];
    double ori_y = initPoint[5];
    double ori_z = initPoint[6];
    INT32 i_pose_x = pos_x * 1e6;
    INT32 i_pose_y = pos_y * 1e6;
    INT32 i_pose_z = pos_z * 1e6;
    INT32 i_ori_w = ori_w * 1e6;
    INT32 i_ori_x = ori_x * 1e6;
    INT32 i_ori_y = ori_y * 1e6;
    INT32 i_ori_z = ori_z * 1e6;
    *(INT32*)(&init_point_msg[idx]) = i_pose_x;
    idx += sizeof(INT32);
    *(INT32*)(&init_point_msg[idx]) = i_pose_y;
    idx += sizeof(INT32);
    *(INT32*)(&init_point_msg[idx]) = i_pose_z;
    idx += sizeof(INT32);
    *(INT32*)(&init_point_msg[idx]) = i_ori_w;
    idx += sizeof(INT32);
    *(INT32*)(&init_point_msg[idx]) = i_ori_x;
    idx += sizeof(INT32);    
    *(INT32*)(&init_point_msg[idx]) = i_ori_y;
    idx += sizeof(INT32);    
    *(INT32*)(&init_point_msg[idx]) = i_ori_z;
    idx += sizeof(INT32);

    *(UINT16*)(&init_point_msg[idx]) = (UINT16)crc16(&init_point_msg[SND_MSG_HEAD_LEN], idx-SND_MSG_HEAD_LEN);  //crc16
    idx += CRC16_BYTES;
    
    GCS_AddMsgToSendBuffer(init_point_msg, idx);
    ROS_INFO("SEND INIT POSE POINT SUCCESS!");
}

void send_move_base_simple_goal_cmd(const std::vector<double> target_pose)
{
  static char target_pose_msg[16];
  size_t idx = 0;
  target_pose_msg[idx++] = 0xfe;
  target_pose_msg[idx++] = 0xff;
  *(UINT32*)(&target_pose_msg[idx]) = (UINT32)(2 + 8 + CRC16_BYTES);
  idx += sizeof(UINT32);
  target_pose_msg[idx++] = GROUND_STATION_TO_FLIGHT;
  target_pose_msg[idx++] = MESSAGE_ANALYSIS_SET_TARGET_POSE;
  double target_x = target_pose[0];
  double target_y = target_pose[1];
  INT32 x = target_x * 1e6;
  INT32 y = target_y * 1e6;
  *(INT32*)(&target_pose_msg[idx]) = x;
  idx += sizeof(INT32);
  *(INT32*)(&target_pose_msg[idx]) = y;
  idx += sizeof(INT32);
  
  *(UINT16*)(&target_pose_msg[idx]) = (UINT16)crc16(&target_pose_msg[SND_MSG_HEAD_LEN], idx-SND_MSG_HEAD_LEN);  //crc16
  idx += CRC16_BYTES;
  GCS_AddMsgToSendBuffer(target_pose_msg, idx);
}
